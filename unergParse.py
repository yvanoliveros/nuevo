...
def p_command_for(p):
    '''command : PARA ID EQUALS expr TO expr optstep'''
    p[0] = ('PARA',p[2],p[4],p[6],p[7])

def p_command_for_bad_initial(p):
    '''command : PARA ID EQUALS error TO expr optstep'''
    p[0] = "BAD INITIAL VALUE IN PARA STATEMENT"

def p_command_for_bad_final(p):
    '''command : PARA ID EQUALS expr TO error optstep'''
    p[0] = "BAD FINAL VALUE IN PARA STATEMENT"

def p_command_for_bad_step(p):
    '''command : PARA ID EQUALS expr TO expr STEP error'''
    p[0] = "MALFORMED STEP IN PARA STATEMENT"

#### Optional STEP qualifier on PARA statement
